...
15 second cap on a hung auth probe, once per PATH each. 245 assertions as of
2026-09-23, after the ccusage cost cases moved to `archived/fragments/`.
...
| `t_upgrade.sh` | 5 | An install that predates 0.3.4, whose empty `sync.lock`, `cost_cache.lock` and `auth_cache.lock` were flock's fd targets, must not read as permanently locked. The sync deletes the retired `bin/cost_cache_refresh.sh`, and a purge still sweeps the retired cost files and ledgers. Uninstall removes both the old and the new lock names |
