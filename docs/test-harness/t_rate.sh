...
rate() {
  printf '{"five_hour":{"used_percentage":%s,"resets_at":%s},"seven_day":{"used_percentage":%s,"resets_at":%s},"measured_at":%s,"seen":{"plan":"max","windows":["five_hour","seven_day"]},"sessions":{"s1":{"sig":"%s","at":%s}}}\n' \
    "$1" "$five" "$2" "$week" "$3" "$4" "$((now-600))" > "$ST/rate_cache.json"
}
...
usage() {
  printf '{"updated_at":%s,"windows_at":%s,"models":{},"windows":{"five_hour":{"used_percentage":%s,"resets_at":%s},"seven_day":{"used_percentage":%s,"resets_at":%s}}}\n' \
    "$4" "$5" "$1" "$five" "$2" "$3" > "$ST/usage_cache.json"
}
...
eq "the corrected reading is published"        "$(jq -r .seven_day.used_percentage "$ST/rate_cache.json")" "0"
eq "and is stamped with windows_at"            "$(jq -r .measured_at "$ST/rate_cache.json")" "$((now-10))"
...
eq "and publishes nothing"                    "$(jq -r .seven_day.used_percentage "$ST/rate_cache.json")" "41"
auth '"max"'
...
before=$(jq -c . "$ST/rate_cache.json"); bm=$(stat -c %Y "$ST/rate_cache.json" 2>/dev/null || stat -f %m "$ST/rate_cache.json")
run "$(pay 5.0 0 0)" >/dev/null
am=$(stat -c %Y "$ST/rate_cache.json" 2>/dev/null || stat -f %m "$ST/rate_cache.json")
eq "the cache settles after one correction" "$(jq -c . "$ST/rate_cache.json")" "$before"
